...
AJS.Labels=function(b){function p(a,h){if(!a||!h)var d=b("#dialog-label-list"),a=d.attr("entityid"),h=d.attr("entitytype");return!a||!h?b(i.labelView):b(".labels-section-content").filter(function(){return this.getAttribute("entityid")==a&&this.getAttribute("entitytype")==h}).find(i.labelView)}AJS.bind("init.rte-control",function(){var a=0;AJS.Meta.get("num-labels")?a=AJS.Meta.get("num-labels"):b("#createPageLabelsString").val()&&(a=b.trim(b("#createPageLabelsString").val()).split(/\s+/).length);0!=
a&&(AJS.Meta.set("num-labels",a),b("#rte-button-labels").trigger("updateLabel"))});var s=function(a){a=AJS.$.trim(a);if(!a)return[];var h=[],d=(new Date).getTime();b(a.split(/\s+/)).each(function(a,b){var e=b.split(":"),g,k=e[0];2===e.length&&(g=e[0],k=e[1]);h.push({name:k,prefix:g,id:d});d++});return h},i={labelView:".label-list",labelItem:".aui-label",noLabelsMessage:".no-labels-message",idAttribute:"data-label-id",labelsFormFieldId:AJS.Meta.get("labels-form-field-id")||"createPageLabelsString"},
t=Confluence.getContextPath(),l={base:t+"/rest/ui/1.0/",getRestEndPoint:function(a){return this.base+("attachment"==a?"attachment/":"template"==a?"template/":"space"==a?"space/":"content/")},index:t+"/labels/autocompletelabel.action?maxResults=3",validate:function(){return this.getRestEndPoint()+"labels/validate"},add:function(a,b){return this.getRestEndPoint(a)+b+"/labels"},remove:function(a,b,d){return this.getRestEndPoint(a)+b+"/label/"+d},list:function(a,b){return this.getRestEndPoint(a)+b+"/labels"}},
j=function(){return!!document.getElementById("createpagetemplate")},q=function(){return b(".space-administration").length},u=function(a){n();a&&a.promise&&(a.always([w,x]),a.done(function(a,d){AJS.Meta.set("num-labels",p().first().find(i.labelItem).size());b("#rte-button-labels").trigger("updateLabel");var c=d.not(".editable");0===d.find(".aui-label").length?c.find(".labels-edit-container").before(Confluence.Templates.Labels.nolabels()):c.find(".no-labels-message").remove()}));return a},x=function(){b("#labels-string, #add-labels-editor-button").removeAttr("disabled aria-disabled")},
w=function(){b("#labels-string").val("")},n=function(a){a=a||null;b("#label-operation-error-message").empty().toggle(!!a);AJS.messages.warning("#label-operation-error-message",{body:a})},v=function(a,b){var d="/label/";null!==a&&(void 0!=a&&""!==a)&&(d+=a+"/");var c=encodeURI;var f=b.prefix,e=b.name;return c(d+("global"===f||"team"===f||void 0===f||""===f||null===f?e:f+":"+e))};b(".label-list.editable .aui-icon-close").live("click",function(a){a.preventDefault();a=b("#dialog-label-list");AJS.Labels.removeLabel(b(this).closest("li"),
a.attr("entityid"),a.attr("entitytype"))});AJS.toInit(function(){q()&&(AJS.Labels.bindAutocomplete(),b(".label-list").addClass("editable"))});AJS.bind("editor-heartbeat",function(a,h){if("undefined"!==typeof h.labelsHash){var d=b("#rte-button-labels");if(d.data("labelsHash")!==h.labelsHash){if(!j()){var c=AJS.Meta.get("content-type"),f=Confluence.Editor.Drafts.isNewContent()?AJS.Meta.get("draft-id"):AJS.Meta.get("page-id");b.ajax({url:l.list(c,f),cache:!1,success:function(a){AJS.Meta.set("num-labels",
a.labels.length);b("#rte-button-labels").trigger("updateLabel");AJS.Labels.updateDialogLabelList(a.labels)},error:function(a,b){AJS.logError(b)}})}d.data("labelsHash",h.labelsHash)}}});return{addLabel:function(a,h,d){var c=a;if(c){b("#labels-string, #add-labels-editor-button").attr({disabled:"disabled","aria-disabled":!0});var f=b("#"+i.labelsFormFieldId);if(j()){var a=(f.val()+" "+c).split(/\s+/),e=[];AJS.$.each(a,function(a,b){-1===AJS.$.inArray(b,e)&&e.push(b)});c=e.join(" ")}var a=s(c),a={url:j()?
l.validate():l.add(d,h),type:"POST",dataType:"json",contentType:"application/json",data:JSON.stringify(a)},g=b.Deferred(),a=AJS.$.ajax(a);a.done(function(a){var e=AJS.Meta.get("space-key"),r=b("#dialog-label-list").find(".label-list"),m=p(h,d).not(".editable"),a=a.labels,i=(new Date).getTime();AJS.$.each(a,function(a,b){b.link=v(e,b);j()&&(f.val(c),b.id=i++)});j()&&r.empty();r.html(Confluence.Templates.Labels.dialogLabels({labels:a,editable:!0}));m.find("li.aui-label").remove();m.find("li.labels-edit-container").before(Confluence.Templates.Labels.dialogLabels({labels:a,
spaceKey:e}));g.resolve(a,m.add(r))});a.fail(function(a,b){var c;if("timeout"===b)c="Connecting to the server timed out, you\'ll need to try again";else try{c=JSON.parse(a.responseText).message}catch(d){c=b,AJS.log("Unexpected error when adding labels: "+d.message)}AJS.log("Failed to add labels: "+c);AJS.log("Failed to add labels xhr: "+a.responseText);n(c);g.reject(b)});a=g.promise()}else a=!1;return u(a)},removeLabel:function(a,h,d){var c=a,f=h,e=d;if(c){c=c.jquery?c:b(c);q()&&(f=AJS.Meta.get("space-key"),e="space");var g=c.attr(i.idAttribute),
k=b.trim(c.find("a").text()),a={type:"DELETE",dataType:"json",contentType:"application/json",data:{}},o=b.Deferred();j()?(a=b.Deferred(),a.resolve()):(a.url=l.remove(e,f,g),a=AJS.$.ajax(a));a.done(function(){var a=p(f,e);q()&&(a=b("#space-categories-list"));var d=a.find(i.labelItem).filter("["+i.idAttribute+"='"+g+"']");d.fadeOut("normal",function(){d.remove();o.resolve(c,a)})});a.fail(function(a,b){AJS.log(b);n(b=="timeout"?"Connecting to the server timed out, you\'ll need to try again":b);o.reject(b)});j()&&o.done(function(){var a=
b("#"+i.labelsFormFieldId),c=a.val().split(/\s+/),c=b.grep(c,function(a){return!a||a==k},true);a.val(c.join(" "))});a=o.promise()}else a=!1;return u(a)},bindAutocomplete:function(){var a=b("#labels-string"),h=a.parents("#add-labels-form").length;a.length&&(b(window).bind("quicksearch.ajax-success",function(a,b){if("/labels/autocompletelabel.action?maxResults=3"==b.url)return AJS.Labels.queryTokens=b.json&&b.json.queryTokens||[],!1}),b(window).bind("quicksearch.ajax-error",function(a,b){if("/labels/autocompletelabel.action?maxResults=3"==
b.url)return AJS.Labels.queryTokens=[],!1}),a.quicksearch("/labels/autocompletelabel.action?maxResults=3",function(){if(!b("#labels-autocomplete-list .label-suggestion").length||""===a.val())this.hide();else if(!h)for(var d=b("#labels-autocomplete-list a.label-suggestion"),c=0,f=d.length;c<f;c++)d.get(c).href="#"},{makeParams:function(a){return{query:a,contentId:AJS.params.pageId||""}},dropdownPlacement:function(a){b("#labels-autocomplete-list").append(a)},ajsDropDownOptions:{selectionHandler:function(d,
c){if(c.find("a.label-suggestion").length){var f=b("span",c),f=b.data(f[0],"properties"),e=a.val(),g=[];if(h)g=e.split(/\s+/),g[g.length-1]=f.name,a.val(g.join(" "));else{for(var k=AJS.Labels.queryTokens,g=-1,i,j="",m=0,l=k.length;m<l;m++)j=k[m],i=e.lastIndexOf(j),i>g&&(g=i);-1!=g?(k=e.substr(0,g),(e=e.substr(g+j.length).match(/^\s+/))&&(k+=e[0]),a.val(k+f.name)):a.val(f.name)}}this.hide();d.preventDefault();a.focus()}}}))},isNewTemplate:j,routes:l,setLabelError:n,parseLabelStringToArray:s,getLabelLink:v}}(AJS.$);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
AJS.Labels.openDialog(a):AJS.Labels.openDialog()})};AJS.Labels.openDialog=function(a){var e,d,f;c||(c=AJS.ConfluenceDialog({width:600,height:250,id:"edit-labels-dialog",onCancel:h}),c.addHeader("Labels"),c.addPanel("Label Editor",Confluence.Templates.Labels.dialog()),c.addCancel("Close",h),c.addHelpText("Shortcut tip: In page view, pressing \u003cb\u003e{shortcut}\u003c/b\u003e also opens this dialog box",{shortcut:"l"}),c.popup.element.find(".dialog-title").prepend(Confluence.Templates.Labels.helpLink()),b("#add-labels-form").submit(function(a){var c=
b("#labels-string");a.preventDefault();a=b("#dialog-label-list");AJS.Labels.addLabel(c.val(),a.attr("entityid"),a.attr("entitytype")).always(function(){b("#labels-autocomplete-list").find(".aui-dropdown").addClass("hidden");c.focus()})}),AJS.Labels.bindAutocomplete());a?(d=a.attr("entityid"),e=a.attr("entitytype")):(e=AJS.Meta.get("content-type"),d=Confluence.Editor.Drafts.isNewContent()?AJS.Meta.get("draft-id"):AJS.Meta.get("page-id"));a=AJS.Labels.routes.list(e,d);f=b("#dialog-label-list");f.attr("entityid",
d);f.attr("entitytype",e);var g=function(){c.show();b("#labels-string").val("").focus();AJS.trigger("labels.dialog.open")};d=function(a){AJS.Labels.updateDialogLabelList(a.labels);AJS.Meta.set("num-labels",a.labels.length);b("#rte-button-labels").trigger("updateLabel");g()};AJS.Labels.isNewTemplate()?f.find("ul").children().size()?g():(a=b("#createPageLabelsString").val()||"",a=AJS.Labels.parseLabelStringToArray(a),d({labels:a})):b.ajax({url:a,cache:!1,success:d,error:function(){AJS.Labels.setLabelError("Unable to fetch current labels from the server.");
g()}});b("#dialog-label-list").attr("data-ready","true")};AJS.Labels.updateDialogLabelList=function(a){c&&(AJS.$.each(a,function(a,b){b.link=AJS.Labels.getLabelLink(AJS.Meta.get("space-key"),b)}),b("#dialog-label-list").html(Confluence.Templates.Labels.dialogLabelList({labels:a,editable:!0})))}}});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
